...
def upgrade() -> None:
    ...
    quest_status_enum.create(op.get_bind())
    op.alter_column(
        "questParticipations",
        "status",
        existing_type=sa.INTEGER(),
        type_=quest_status_enum,
        postgresql_using="case WHEN status = 2 THEN 'UNSTARTED'::queststatus \
            WHEN status = 3 THEN 'ACTIVE'::queststatus \
            WHEN status = 4 THEN 'FINISHED'::queststatus \
            ELSE 'HIDDEN'::queststatus END",
...
def downgrade() -> None:
    ...
    op.alter_column(
        "questParticipations",
        "status",
        existing_type=quest_status_enum,
        type_=sa.INTEGER(),
    ...
    quest_status_enum.drop(op.get_bind())
    # ### end Alembic commands ###
